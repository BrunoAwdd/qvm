...
impl QLangParser {
    ...
    pub fn parse_line(&self, line: &str) -> Result<QLangCommand, String> {
        ...
        let canonical_name = resolve_alias(raw);

        match canonical_name {
            "create" => self.parse_create(args),
            "display" => Ok(QLangCommand::Display),
            "measure" => self.parse_measure(args),
            "run" => Ok(QLangCommand::Run),
            "reset" => Ok(QLangCommand::Reset),
            "measure_all" => Ok(QLangCommand::MeasureAll),
            other => self.parse_gate(other, args),
        }
    }
    ...
    fn parse_measure(&self,args: Vec<String>) -> Result<QLangCommand, String> {
        if args.is_empty() {
            ...
            let qubits: Result<Vec<usize>, _> = args.iter().map(|a| {
                a.parse::<usize>().map_err(|_| format!("Invalid qubit: {}", a))
            }).collect();
            Ok(QLangCommand::MeasureMany(qubits?))
        }
    }

    fn parse_gate(&self, name: &str, args: Vec<String>) -> Result<QLangCommand, String> {
        Ok(QLangCommand::ApplyGate(name.to_string(), args))
    }
