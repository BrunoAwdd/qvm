...
impl ParserInternal {
    ...
    fn parse_command(&mut self) -> Result<QLangCommand, String> {
        ...
        if self.match_token(&[Token::Break]) {
            return Ok(QLangCommand::Break);
        }
        if self.match_token(&[Token::Continue]) {
            return Ok(QLangCommand::Continue);
        }
            ...
            if !self.check(Token::RParen) {
                loop {
                    match self.parse_expression()? {
                        ...
                        _ => {
                            return Err("measure() expects number literals for qubit indices".into())
                        }
                    }
    ...
    fn parse_type(&mut self) -> Result<TypeAnnotation, String> {
        ...
        match name.as_str() {
            ...
            other => Err(format!(
                "Unknown type '{}'. Valid types: qubit, bit, int, float, bool, void, [T]",
                other
            )),
        }
    }
    ...
    fn parse_term(&mut self) -> Result<Expression, String> {
        ...
        while self.match_token(&[Token::Plus, Token::Minus]) {
            let op = if matches!(self.previous(), Token::Plus) {
                Operator::Add
            } else {
                Operator::Sub
            };
    ...
    fn parse_factor(&mut self) -> Result<Expression, String> {
        ...
        while self.match_token(&[Token::Star, Token::Slash]) {
            let op = if matches!(self.previous(), Token::Star) {
                Operator::Mul
            } else {
                Operator::Div
            };
