...
fn execute_command(
    ...
    match cmd {
        ...
        QLangCommand::While { condition, body } => loop {
            if !evaluate_expression(condition, variables, qvm, functions).is_truthy() {
                break;
            }
...
fn evaluate_expression(
    ...
    match expr {
        ...
        Expression::Variable(name) => variables.get(name).cloned().unwrap_or_else(|| {
        ...
        Expression::Array(exprs) => Value::Array(
            exprs
                .iter()
                .map(|e| evaluate_expression(e, variables, qvm, functions))
                .collect(),
        ),
        Expression::Index(arr_expr, idx_expr) => {
            ...
            match evaluate_expression(arr_expr, variables, qvm, functions) {
                ...
                _ => {
                    eprintln!("Error: cannot index a non-array value");
                    Value::Number(0.0)
                }
            }
        }
...
fn resolve_args(
    ...
    for arg in args {
        match evaluate_expression(arg, variables, qvm, functions) {
            ...
            Value::Array(_) => {
                eprintln!(
                    "Error: gate '{}' does not accept array arguments",
                    gate_name
                );
                return None;
            }
