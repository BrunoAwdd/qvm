...
fn execute_command(
    ...
    match cmd {
        ...
        QLangCommand::QIf { condition, then_branch, else_branch } => {
            let control_qubit = evaluate_expression(condition, variables, qvm).as_usize();

            // Then Branch
            for cmd in then_branch {
                if let QLangCommand::ApplyGate(name, args) = cmd {
                    let mut new_args = vec![control_qubit.to_string()];
                    for arg in args {
                        let val = evaluate_expression(arg, variables, qvm);
                        match val {
                            Value::Number(n) => new_args.push(n.to_string()),
                            _ => panic!("Array args not supported in qif yet"),
                        }
                    }
                    apply_controlled_gate(qvm, name, &new_args);
                } else {
                    println!("Warning: Only quantum gates are supported inside qif");
                }
            }
            ...
            if let Some(else_cmds) = else_branch {
                // Apply X to control
                qvm.apply_gate(&PauliX::new(), control_qubit);
...
mod tests {
    ...
    fn test_variable_assignment() {
        ...
        run_ast(&mut qvm, &ast, &mut vars, &mut funcs);
        assert_eq!(*vars.get("a").unwrap(), Value::Number(10.0));
    }
    ...
    fn test_if_condition() {
        ...
        let ast = vec![
            ...
            QLangCommand::If {
                ...
                then_branch: vec![QLangCommand::ApplyGate("x".into(), vec![Expression::Number(0.0)])],
                else_branch: None
            }
        ];
        run_ast(&mut qvm, &ast, &mut vars, &mut funcs);
    ...
    #[test]
    fn test_qif_execution() {
        let mut qvm = QVM::new(2);
        let mut vars = HashMap::new();
        let mut funcs = HashMap::new();
        
        // |+>|0>
        // qif(0) { x(1) } -> Bell state (|00> + |11>)/sqrt(2)
        let ast = vec![
            QLangCommand::Create(2),
            QLangCommand::ApplyGate("h".into(), vec![Expression::Number(0.0)]),
            QLangCommand::QIf {
                condition: Expression::Number(0.0),
                then_branch: vec![QLangCommand::ApplyGate("x".into(), vec![Expression::Number(1.0)])],
                else_branch: None,
            }
        ];
        run_ast(&mut qvm, &ast, &mut vars, &mut funcs);
        ...
        assert!((state[3].norm_sqr() - 0.5).abs() < 1e-6);
    }
    ...
    fn test_qif_else() {
        ...
        // |0>|0>
        // qif(0) { x(1) } else { x(1) } -> |0>|1> because control is 0 so else runs
        let ast = vec![
            QLangCommand::Create(2),
            QLangCommand::QIf {
                condition: Expression::Number(0.0),
                then_branch: vec![QLangCommand::ApplyGate("x".into(), vec![Expression::Number(1.0)])],
                else_branch: Some(vec![QLangCommand::ApplyGate("x".into(), vec![Expression::Number(1.0)])]),
            }
        ];
        run_ast(&mut qvm, &ast, &mut vars, &mut funcs);
        ...
        let state = qvm.state_vector();
        // |q1 q0> = |1 0> = 2
        assert!((state[2].norm_sqr() - 1.0).abs() < 1e-6); 
    }
}
